...
def _coverage_repeat_during_traffic(conn: psycopg.Connection[tuple[object, ...]]) -> None:
    with psycopg.connect(conn.info.dsn) as traffic, psycopg.connect(conn.info.dsn, autocommit=True) as installer:
...
def test_coverage_installer_rejects_incompatible_state_without_replacing_it(
    ...
    state_query: Final = (
        "SELECT prosrc,provolatile,obj_description(oid,'pg_proc') FROM pg_proc "
        "WHERE oid='litellm_update_daily_autorouter_coverage()'::regprocedure"
    )
...
def test_waiting_coverage_installer_rechecks_after_peer_finishes_before_taking_table_locks(
    ...
    with (
        psycopg.connect(_coverage_postgresql.info.dsn) as original_reader,
        psycopg.connect(_coverage_postgresql.info.dsn) as next_reader,
        psycopg.connect(_coverage_postgresql.info.dsn, autocommit=True) as first,
        psycopg.connect(_coverage_postgresql.info.dsn, autocommit=True) as second,
