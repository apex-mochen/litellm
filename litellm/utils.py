...
def validate_chat_completion_tool_choice(
    ...
    if tool_choice is None or isinstance(tool_choice, str):
        ...
        # OpenAI Chat Completions spec (gpt-5+):
        # {"type": "allowed_tools", "allowed_tools": {"mode": "auto"|"required",
        #  "tools": [...]}}. This is not a per-function choice; pass it through to
        # the provider as-is rather than flattening or rejecting it.
        if tool_choice_type == "allowed_tools":
            if not isinstance(tool_choice.get("allowed_tools"), dict):
